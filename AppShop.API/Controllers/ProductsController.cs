...
namespace AppShop.API.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<ActionResult> PostAsync(ProductDTO productDTO)
        {
            try
            {
                ...
                foreach (var productImage in productDTO.ProductImages!)
                {
                    ...
                    newProduct.ProductImages.Add(new ProductImage { Image = await _fileStorage.SaveFileAsync(photoProduct, ".jpg", "products").ConfigureAwait(false) });
                }

                foreach (var productCategoryId in productDTO.ProductCategoryIds!)
                {
                    var category = await _context.Categories.FirstOrDefaultAsync(x => x.Id == productCategoryId).ConfigureAwait(false);
            ...
            catch (DbUpdateException dbUpdateException)
            {
                ...
                {
                    return BadRequest("There is already exist a product with that exact name.");
                }
        ...
        {
            var product = await _context.Products.FirstOrDefaultAsync(x => x.Id == id).ConfigureAwait(false);
            if (product == null)
            {
                return NotFound();
            }
